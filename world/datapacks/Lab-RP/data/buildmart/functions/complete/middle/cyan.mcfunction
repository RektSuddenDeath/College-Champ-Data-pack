...
# Add Completions
scoreboard players add cyan bm_completions 1

scoreboard players add cyan bm_normalcomps 1
...
scoreboard players operation cyan bm_teamscore += @s bm_earnScore

# Text and visuals
summon firework_rocket ~-2 ~4 ~3 {Life:10000,LifeTime:0,FireworksItem:{id:"firework_rocket",Count:1,tag:{Fireworks:{Flight:-2,Explosions:[{Type:0,Flicker:true,Colors:[I;23396]}]}}}}

tellraw @a[team=!cyan] ["","§3青队","§7第",{"score":{"name": "@s","objective": "bm_compNo"},"color": "yellow"},"§7个完成了","§a普通建筑 #",{"score":{"name": "@s","objective": "bm_plotNo"},"color":"green"}]
tellraw @a[team=cyan] ["[§6+§r",{"score":{"name": "@s","objective": "bm_earnScore"},"color": "gold"},"§6分§f] ","§7你","§7第",{"score":{"name": "@s","objective": "bm_compNo"},"color": "yellow"},"§7个完成了","§a普通建筑 #",{"score":{"name": "@s","objective": "bm_plotNo"},"color":"green"}]

execute as @a[team=cyan] at @s run playsound minecraft:sound.bigcoins record @s
execute as @a[team=cyan] at @s run playsound minecraft:sound.acquire record @s
# After Operations

fill ~ ~1 ~ ~6 ~6 ~6 air replace
execute as @e[type=marker,tag=zone_middle,tag=cyan] run scoreboard players operation @s bm_midplotNo = cyan bm_normalcomps
execute as @e[type=marker,tag=zone_middle,tag=cyan] run scoreboard players operation @s bm_midplotNo += 2 math

execute as @e[type=marker,tag=zone_middle,tag=cyan,tag=bm_example,limit=1,sort=nearest] at @s run function buildmart:copy/find_normal_middle
