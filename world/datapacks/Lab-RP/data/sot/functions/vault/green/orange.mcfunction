...
tellraw @a[team=orange] ["",{"selector":"@p[team=orange,gamemode=adventure]"},"§6开启了§2绿色宝库§6！"]
tellraw @a[tag=!admin] ["",{"translate":"team.orange"},"§e开启了一个宝库！"]
tellraw @a[tag=admin] ["",{"translate":"team.orange"},"§e开启了§2绿色宝库§e！"]

execute as @a[team=orange] at @s run playsound sound.sands_of_time.vault_open_ally record @s
scoreboard players set orange.G SotVault 0