...
# 开启和关闭门
execute as @e[type=minecraft:area_effect_cloud,tag=gr_purpleanchor] at @s run fill ~51 ~10 ~14 ~51 ~14 ~16 barrier
## Summon general aecs
execute as @e[type=minecraft:area_effect_cloud,tag=gr_purpleanchor] at @s run summon area_effect_cloud ~51 ~14 ~15 {Duration:9999999,Tags:["gr_closer"]}

execute as @e[type=minecraft:area_effect_cloud,tag=gr_purpleanchor] at @s run summon area_effect_cloud ~45 ~10 ~15 {Duration:9999999,Tags:["gr_opener"]}
...
title @a[team=purple] title "§e前往终点吧！"
# Give items
clear @a[team=purple]
# 开始计时
