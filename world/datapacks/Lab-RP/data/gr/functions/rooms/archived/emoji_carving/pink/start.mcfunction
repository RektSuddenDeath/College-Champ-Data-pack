...
# Modify scoreboard
scoreboard players display name var_room_name gr_pinksb "§a§l当前房间: §rEmoji Carving"
...
# Give items
clear @a[team=pink]
execute as @e[type=minecraft:area_effect_cloud,tag=gr_pinkanchor] at @s positioned ~33 ~10 ~21 as @a[dx=3,dy=3,dz=4,team=pink,limit=2] run give @s iron_pickaxe{Unbreakable:1b,CanDestroy:[yellow_concrete]}
# 开始计时
