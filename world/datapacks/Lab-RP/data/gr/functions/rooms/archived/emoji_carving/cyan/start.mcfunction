...
# Modify scoreboard
scoreboard players display name var_room_name gr_cyansb "§a§l当前房间: §rEmoji Carving"
...
title @a[team=cyan] title "§e复制图案！"
# Give items
clear @a[team=cyan]
execute as @e[type=minecraft:area_effect_cloud,tag=gr_cyananchor] at @s positioned ~33 ~10 ~21 as @a[dx=3,dy=3,dz=4,team=cyan,limit=2] run give @s iron_pickaxe{Unbreakable:1b,CanDestroy:[yellow_concrete]}
# 开始计时
