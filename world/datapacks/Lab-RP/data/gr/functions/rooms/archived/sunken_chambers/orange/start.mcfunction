...
# Status Effects
scoreboard players set @a[team=orange] univ_death 0
effect clear @a[team=orange] resistance
effect give @a[team=orange] absorption infinite 1 true
execute as @a[team=orange] run attribute @s generic.max_health base set 1.0

# Add a team phase
scoreboard players add orange gr_teamphase 1

# Modify scoreboard
scoreboard players display name var_room_name gr_orangesb "§a§l当前房间: §rSunken Chambers"
