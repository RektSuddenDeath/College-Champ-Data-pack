...
tellraw @a[gamemode=survival] ["","[",{"score":{"name": "win","objective": "sba_stats"},"color": "gold"},{"text": "分","color": "gold"},"] §a[§r彀§a]",{"text": "你是最后存活的玩家！"}]
...
execute if score eventmode tourney_stats matches 0 run scoreboard players set §4 sba_sb 13
...
execute if score map sba_game matches 1 run schedule function skybattle:structure/zone3/classic/place 35s
execute if score map sba_game matches 2 run schedule function skybattle:structure/zone3/sandcastle/place 35s
execute if score map sba_game matches 3 run schedule function skybattle:structure/zone3/wasteland/place 35s
execute if score map sba_game matches 4 run schedule function skybattle:structure/zone3/cakesland/place 35s
