...
tellraw @a[gamemode=survival] ["","[§6+",{"score":{"name": "win","objective": "sba_stats"},"color": "gold"},{"text": "分","color": "gold"},"] §a[§r彀§a]",{"text": "你是最后存活的玩家！"}]
...
execute if score map sba_game matches 1 run schedule function skybattle:structure/zone2/classic/place 10s
execute if score map sba_game matches 2 run schedule function skybattle:structure/zone2/sandcastle/place 10s
execute if score map sba_game matches 3 run schedule function skybattle:structure/zone2/wasteland/place 10s
execute if score map sba_game matches 4 run schedule function skybattle:structure/zone2/cakesland/place 10s