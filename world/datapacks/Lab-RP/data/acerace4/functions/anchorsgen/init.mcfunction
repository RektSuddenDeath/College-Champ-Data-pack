...
kill @e[type=area_effect_cloud,tag=posAnchor,tag=map4]
tag @s add gen
...
tellraw @s "Use §a/function acerace4:anchorsgen/stop§r to stop generation when you are near the finish line."
tellraw @s "§c§lDO NOT CROSS THE FINISH LINE FOR ACCURACY"
tellraw @s "-----------"

function acerace4:anchorsgen/loop