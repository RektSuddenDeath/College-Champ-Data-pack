...
execute as @e[type=marker,tag=zone_left,tag=yellow] run scoreboard players operation @s bm_leftplotNo = yellow bm_normalcomps
execute as @e[type=marker,tag=zone_left,tag=yellow] run scoreboard players operation @s bm_leftplotNo += 2 math

execute as @e[type=marker,tag=zone_left,tag=yellow,tag=bm_example,limit=1,sort=nearest] at @s run function buildmart:copy/find_normal_left
