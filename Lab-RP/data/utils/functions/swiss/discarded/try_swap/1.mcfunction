...
# LF MATCH
tag @s add lf_match
data modify storage swiss_sort match_to_swap set value 1

# 找lower placed team
scoreboard objectives add swiss_swap_temp dummy
scoreboard players set temp_order swiss_swap_temp 99999
execute as @e[type=marker,tag=swiss_sort,scores={swiss_match_result=1}] run scoreboard players operation temp_order swiss_swap_temp < @s swiss_order_asc

# 对高低排名都做标记
execute as @e[type=marker,tag=swiss_sort,scores={swiss_match_result=1}] if score @s swiss_order_asc = temp_order swiss_swap_temp run tag @s add swiss_temp_lower
execute as @e[type=marker,tag=swiss_sort,scores={swiss_match_result=1}] unless score @s swiss_order_asc = temp_order swiss_swap_temp run tag @s add swiss_temp_higher
