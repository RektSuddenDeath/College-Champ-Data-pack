...
# Modify scoreboard
scoreboard players display name var_room_name gr_yellowsb "§a§l当前房间: §rSink Ground"
...
title @a[team=yellow] title "§e到达另一边！"
# Give items
clear @a[team=yellow]

execute as @e[type=minecraft:area_effect_cloud,tag=gr_yellowanchor] at @s run summon item ~4 108 ~5 {Age:-32767s,Item:{Count:1b,id:"leather_boots",tag:{Unbreakable:1b}}}
execute as @e[type=minecraft:area_effect_cloud,tag=gr_yellowanchor] at @s run summon item ~22 106 ~10 {Age:-32767s,Item:{Count:1b,id:"leather_boots",tag:{Unbreakable:1b}}}
execute as @e[type=minecraft:area_effect_cloud,tag=gr_yellowanchor] at @s run summon item ~14 94 ~20 {Age:-32767s,Item:{Count:1b,id:"leather_boots",tag:{Unbreakable:1b}}}
