...
# Modify scoreboard
scoreboard players display name var_room_name gr_purplesb "§a§l当前房间: §rAncient City"
...
title @a[team=purple] title "§e点亮红石灯！"
# Give items
clear @a[team=purple]
give @a[team=purple] iron_sword{Unbreakable:1b}
effect clear @a[team=purple] night_vision
execute as @e[type=minecraft:area_effect_cloud,tag=gr_purpleanchor] at @s run summon warden ~16 ~9 ~15
execute as @e[type=warden,tag=!grroom] run data modify entity @s Health set value 50f
execute as @e[type=minecraft:area_effect_cloud,tag=gr_purpleanchor] at @s run tag @e[type=warden,tag=!grroom] add purple
execute as @e[type=minecraft:area_effect_cloud,tag=gr_purpleanchor] at @s run tag @e[type=warden,tag=!grroom] add grroom
# 开始计时
