...
# Modify scoreboard
scoreboard players display name var_room_name gr_greensb "§a§l当前房间: §rAncient City"
...
effect clear @a[team=green] night_vision
execute as @e[type=minecraft:area_effect_cloud,tag=gr_greenanchor] at @s run summon warden ~16 ~9 ~15
execute as @e[type=warden,tag=!grroom] run data modify entity @s Health set value 50f
execute as @e[type=minecraft:area_effect_cloud,tag=gr_greenanchor] at @s run tag @e[type=warden,tag=!grroom] add green
execute as @e[type=minecraft:area_effect_cloud,tag=gr_greenanchor] at @s run tag @e[type=warden,tag=!grroom] add grroom
# 开始计时
