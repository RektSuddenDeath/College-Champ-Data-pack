...
# Status Effects
scoreboard players set @a[team=yellow] univ_death 0
effect clear @a[team=yellow] resistance
effect give @a[team=yellow] absorption infinite 1 true
execute as @a[team=yellow] run attribute @s generic.max_health base set 1.0

# Add a team phase
scoreboard players add yellow gr_teamphase 1

# Modify scoreboard
scoreboard players display name var_room_name gr_yellowsb "§a§l当前房间: §rSunken Chambers"
