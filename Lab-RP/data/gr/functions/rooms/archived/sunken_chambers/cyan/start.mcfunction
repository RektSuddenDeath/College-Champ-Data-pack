...
# Status Effects
scoreboard players set @a[team=cyan] univ_death 0
effect clear @a[team=cyan] resistance
effect give @a[team=cyan] absorption infinite 1 true
execute as @a[team=cyan] run attribute @s generic.max_health base set 1.0

# Add a team phase
scoreboard players add cyan gr_teamphase 1

# Modify scoreboard
scoreboard players display name var_room_name gr_cyansb "§a§l当前房间: §rSunken Chambers"
