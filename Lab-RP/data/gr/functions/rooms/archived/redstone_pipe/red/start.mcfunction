...
# Modify scoreboard
scoreboard players display name var_room_name gr_redsb "§a§l当前房间: §rRedstone Pipe"
...
# Give items
clear @a[team=red]
give @a[team=red] shears{Unbreakable:1b,display:{Name:'"§c红石剪刀 §r（可用于剪下红石线）"'},CanDestroy:[redstone_wire]}
# 开始计时
