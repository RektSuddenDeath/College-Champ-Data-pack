...
execute as @a at @s if block ~ ~-1 ~ gold_block run effect give @s jump_boost 1 9 true
execute as @a at @s if block ~ ~-1 ~ gold_block run title @s times 0 2 0
execute as @a at @s if block ~ ~-1 ~ gold_block run title @s title ""
execute as @a at @s if block ~ ~-1 ~ gold_block run title @s subtitle "§e⇈"
execute as @a[nbt={OnGround:1b}] at @s if block ~ ~-1 ~ gold_block run tag @s add ar.jumppad



execute if score gameflag ar_game matches 1 run schedule function acerace4:effectblock/jump10 1t